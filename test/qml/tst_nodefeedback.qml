...
TestCase {
    ...
    function init() {
        ...
        nodeModel.answerRuntimeDialog(false)
    }
    ...
    function test_runtime_dialog_opens_from_node_model_and_answers() {
        ...
        nodeModel.setRuntimeDialogForTest("Question", "Continue?", true)
        tryCompare(popup, "opened", true)

        const title = findChild(popup, "nodeRuntimeDialogTitle")
        const message = findChild(popup, "nodeRuntimeDialogMessage")
        const primary = findChild(popup, "nodeRuntimeDialogPrimaryButton")
        const secondary = findChild(popup, "nodeRuntimeDialogSecondaryButton")
        verify(title !== null)
        verify(message !== null)
        verify(primary !== null)
        ...
        mouseClick(primary, primary.width / 2, primary.height / 2)
    ...
    function test_runtime_dialog_wraps_long_message() {
        ...
        nodeModel.setRuntimeDialogForTest("Warning", longWarningText(), false)
        tryCompare(popup, "opened", true)
