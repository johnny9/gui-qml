...
QString RuntimeDialogPrimaryText(unsigned int style, bool question)
{
    if (style & CClientUIInterface::BTN_YES) return QObject::tr("Yes");
    if (style & CClientUIInterface::BTN_RETRY) return QObject::tr("Retry");
    if (style & CClientUIInterface::BTN_APPLY) return QObject::tr("Apply");
    if (question) return QObject::tr("Yes");
    return QObject::tr("OK");
}

QString RuntimeDialogSecondaryText(unsigned int style, bool question)
{
    if (style & CClientUIInterface::BTN_NO) return QObject::tr("No");
    if (style & CClientUIInterface::BTN_CANCEL) return QObject::tr("Cancel");
    if (style & CClientUIInterface::BTN_ABORT) return QObject::tr("Abort");
    if (style & CClientUIInterface::BTN_CLOSE) return QObject::tr("Close");
    if (style & CClientUIInterface::BTN_DISCARD) return QObject::tr("Discard");
    return question ? QObject::tr("No") : QString{};
}
...
QVariantList NodeModel::nodeInformationRows()
{
    ...
    const int block_height{std::max(m_block_tip_height, m_node.getNumBlocks())};
    const int64_t last_block_time{m_node.getLastBlockTime()};
    ...
    rows.push_back(InformationRow(tr("Network active"), m_node.getNetworkActive() ? tr("Yes") : tr("No")));
